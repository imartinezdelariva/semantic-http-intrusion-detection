...
# =============================================
# DATOS PARA TRAFICO NORMAL HIGH
# Nombres y user agents distintos a low y medium
# =============================================

NOMBRES = ["Kenji", "Yuna", "Mateo", "Valentina", "Liam",
           "Olivia", "Noah", "Ava", "Elijah", "Mia",
           "James", "Luna", "Aiden", "Chloe", "Lucas",
           "Penelope", "Mason", "Riley", "Ethan", "Zoey"]
...
def generar_trafico_normal_high(session, n=100):
    print("\n[*] Generando trafico normal HIGH...")
    ok = 0
    ...
    for _ in range(n):
        ...
        try:
            if tipo == "navegacion":
                ...
                r = session.get(url)
                print(f"  NORMAL_H | GET {url:<45} | status: {r.status_code}")

            elif tipo == "sqli_normal":
                # En high el formulario usa token CSRF
                ...
                print(f"  NORMAL_H | SQLi id={id_val:<5} | status: {r.status_code}")

            elif tipo == "xss_normal":
                nombre = random.choice(NOMBRES)
                r = session.get(f"{BASE_URL}/vulnerabilities/xss_r/",
                                params={"name": nombre})
                print(f"  NORMAL_H | XSS nombre={nombre:<15} | status: {r.status_code}")
                ...
                print(f"  NORMAL_H | BRUTE {usuario}:{password:<16} | status: {r.status_code}")

            elif tipo == "exec_normal":
                # Solo IPs validas para trafico normal
                ip = f"127.0.{random.randint(0,1)}.{random.randint(0,10)}"
                r = session.post(f"{BASE_URL}/vulnerabilities/exec/",
                                 data={"ip": ip, "Submit": "Submit"})
                print(f"  NORMAL_H | EXEC ip={ip:<18} | status: {r.status_code}")
    ...
    print(f"[+] Trafico normal High completado: {ok}/{n} peticiones enviadas")
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("  ENTRENAMIENTO: TRAFICO NORMAL — NIVEL HIGH")
    ...
    generar_trafico_normal_high(session, n=100)
